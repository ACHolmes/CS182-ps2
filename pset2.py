...
class SudokuGameBoard:
    ...
    def get_related_coords(self, coord:tuple)->list:
        ...
        x, y = coord
        coords = []
        # Adding all elts in row or column
        for i in range(9):
            if i != x:
                coords.append((i, y))
            if i != y:
                coords.append((x, i))
        # Adding elts in our box
        xblock = 3 * int(x / 3)
        yblock = 3 * int(y / 3)
        for i in range(xblock, xblock + 3):
            for j in range(yblock, yblock + 3):
                if ((i, j) != (x, y)):
                    coords.append((i, j))
        return coords
    ...
    def fwd_checking(self):
        """A method to perform forward checking that enforces arc-consistency to reduce the possiable legal candidates remaining for each unassigned decision variable
...
def MRV_heuristic(coordinate_dict:dict)->tuple:
    ...
    minoptions = 10
    bestpos = (0, 0)
    for pos, options in coordinate_dict.items():
